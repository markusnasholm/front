...
export const Header: React.FC<HeaderProps> = ({ licenseEdition, licenseExpired, licenseEnd, licenseCurrentNodeCount, licenseNodeLimit }) => {
  return <div className="header">
    <div style={{ display: "flex", alignItems: "center" }}>
      ...
      {licenseEdition !== "community" && !licenseExpired && <div className={`extra ${licenseCurrentNodeCount > licenseNodeLimit ? "failure" : "success" }`}>{`Node limit: ${licenseCurrentNodeCount}/${licenseNodeLimit}`}</div>}
    </div>
    <div style={{ display: "flex", alignItems: "center" }}>
      <UI.InformationIcon />
    </div>
